...
# Runs whatever build is installed on the simulator; `make ns-mac-qa` first to refresh it.
...
(( $# > 0 )) || { sed -n '2,10p' "$0" | sed 's/^# \{0,1\}//'; exit 64; }
...
make -C "$REPO_ROOT" ns-mac-flow FLOW="${flows[*]}"
