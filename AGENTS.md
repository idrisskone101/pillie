...
- `namespace-mac` — on-demand Namespace macOS Devbox. **Load this on a Linux Cloud Agent before any Xcode or simulator step.** Prefer `make ns-mac-verify CMD='…'`. For a batch, start `pillie-ios`, verify, then stop it. Do not start it for edits or planning. Do not leave it running.
