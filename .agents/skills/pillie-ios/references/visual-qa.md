...
1. Boot the simulator, then `make build-and-run` (or `make run` if already built). `build-and-run` does not boot; install fails with `SimError 405 Shutdown` until `xcrun simctl boot "$UDID"` and `bootstatus -b` succeed. On a Linux Cloud Agent, `make ns-mac-screenshot` does this on `pillie-ios`. `namespace-mac` start/exec installs `axe` on that Mac and puts Homebrew on the remote PATH.
