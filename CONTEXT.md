...
**Due Action Window / Today**:
The live day in Pillie is the 24-hour span from one Due Action Reminder to the next, not the civil calendar day that starts at midnight. A 9:00 PM reminder stays completable until the next 9:00 PM. Home, History, Settings cycle day, streak, blocking, and reminder catch-up all use this same `today`. After midnight and before the next reminder, yesterday remains today if that dose is still due. The day becomes missed only when the next reminder arrives still untaken.
...
**Last Call Reminder**:
Retired. The end-of-day Last Call / Last ping backstop was removed when [[Due Action Window / Today]] replaced midnight rollover. Interval and Repeats are the only follow-up path. Kept as a historical term — it once meant a single Plus-gated re-fire at a configurable evening time, independent of the Auto-Reminder Retry cadence.
_Avoid_: Treating Last Call as a current setting or notification kind
...
Retired (issue #204). This was the standalone onboarding announcement that replaced the [[Soft Onboarding Paywall]], but production data showed it was the flow's largest conversion leak. The [[Reminder Plan]] now proceeds directly to Plus App Blocking setup, where the [[Reverse Trial]] is granted and a compact, non-blocking disclosure states that Plus is unlocked for 14 days with no card required. The persisted step identity remains only for safe migration into app-blocking setup.
