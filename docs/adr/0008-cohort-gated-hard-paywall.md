...
Status: accepted (supersedes ADR 0007's free-forever and one-shot trial-end decisions for new installs)

Production evidence through 2026-08-01 showed that retained users value Pillie, but the optional free exit removes the purchase decision: 35 of 35 expired-trial users chose Continue Free and none purchased. Pillie will keep the 14-day Reverse Trial and end the free tier for every new install, while grandfathering existing installations. PRD: issue #257.

## Decisions and reasons

**Terms are fixed when the grant is persisted.** `trialGrantDate` remains the read-only Keychain-backed clock input. A fresh install receives hard-paywall terms; pre-existing app state is grandfathered into legacy terms. The app records an immutable install assignment before onboarding state can change, and persists it beside the grant in the Keychain, so an existing installation or trial never changes cohorts mid-trial or after reinstall. Existing-user update grants are explicitly legacy.
...
**Legacy remains legacy.** A grandfathered trial still reaches the dismissible trial-end offer, may Continue Free, and retains the existing reminder-only behavior. Already-expired users remain usable and can reach the lifetime option from ordinary Plus upsells.
...
**RevenueCat offering metadata is the kill switch.** The current offering's Boolean `hard_paywall_enabled` is refreshed on launch. Explicit `false` restores legacy terms for hard-paywall cohorts without an app update, including a one-time legacy expiry surface after a hard wall was previously shown. Missing or malformed metadata keeps the ratified hard-paywall default enabled. Auto-presentation waits for the first configuration refresh so an active rollback cannot briefly show the hard wall from stale launch state.
...
- Shipping at any time is safe because the immutable installation assignment selects hard-paywall terms for new installs and legacy terms for grandfathered installs.
