...
**Expiry at local midnight after the 14th full hormone-active day.** Grant day is a bonus counted day. Scheduled break / off-week days do not consume the 14; Plus stays on and the badge freezes. Hormone-active is the dose engine's `!isBreak` (patch/ring remove counts; `PillPack.isBreakDay` does not). Never mid-blocking-window: the last trial day is fully protected and warning copy can honestly say "tonight." Day-10/13 warnings fire 5 and 2 local days before that expiry, not grant+10/+13. Blocking must never outlive Plus Access even if the app is not opened after expiry — the shield/DeviceActivity side gets a coarse access-valid-until date mirrored into the App Group so it can self-disable; the Keychain copy stays authoritative.
