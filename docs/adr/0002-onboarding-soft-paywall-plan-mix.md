...
> **Superseded in part by [ADR 0007](0007-reverse-trial.md):** The Soft Onboarding Paywall is retired. Onboarding contains no purchase UI — every new user receives a Reverse Trial at a non-purchase Trial Granted Moment, everyone proceeds through the Screen Time branch under Plus Access, and the purchase ask moves to the Trial-End Paywall. This ADR's calibration-step ordering, truthful-copy rules, and completion-state language still stand.
